	...
	private static int bfs(int x, int y) {
		...
		while(!queue.isEmpty()) {
			...
			for(int i =0; i< 4; i++) {
				int nx = cur.x + dx[i];
				int ny = cur.y + dy[i];
				if(nx>=0 && ny>=0 && nx<H && ny<W && visited[nx][ny][cur.k] == false && map[nx][ny] == 0) {
			...
			if(cur.k > 0) {
				for(int i = 0; i< 8; i++) {
					int nx = cur.x + dxh[i];
					int ny = cur.y + dyh[i];
					if(nx>=0 && ny>=0 && nx<H && ny<W && visited[nx][ny][cur.k-1] == false && map[nx][ny] == 0) {
